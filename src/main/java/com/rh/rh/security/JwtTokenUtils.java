...
public class JwtTokenUtils implements Serializable {
	 ...
	 * @return le Token JWT.
	 */
	public String generateToken(Rh rh) {
		Map<String, Object> claims = new HashMap<>();
		claims.put("id", rh.getId());
		claims.put("nom", rh.getNom());
		claims.put("prenoms", rh.getPrenoms());
		claims.put("username", rh.getUsername());
		claims.put("role", rh.getRole().name());
		claims.put("statut", rh.getStatut());
		return doGenerateToken(claims, rh.getUsername());
	}
	...
	private SecretKey getSigningKey() {
		// Convertit la String secrète en octets et crée une clé sécurisée pour HS512
